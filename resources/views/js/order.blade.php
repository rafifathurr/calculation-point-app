<script>
    ...
    function customerCheck() {
        ...
        let type = $('#type').val();
        let type_option = $('#type_option').val();
    ...
    function addMenu(id, id_point) {
        ...
        if (type == 0) {
            ...
            if (id_point != undefined) {
                $.get('{{ url('promo-point') }}/' + id_point, {}).done(function(data) {
                    if ($("#order_item_qty_" + data.promo_point.menu.id + "_" + data.promo_point.id).length ==
                        0) {
                        ...
                        let td_menu_name = $("<td>" +
                            ...
                            "<input type='hidden' name = 'order_item_promo[" + data.promo_point.menu.id +
                            "][menu]'" +
                        ...
                        let td_menu_qty = $("<td>" +
                            "<input type='number' class='form-control text-center' name='order_item_promo[" +
                            data.promo_point.menu.id + "][qty]' " +
                            "id='order_item_qty_" + data.promo_point.menu.id + "_" + data.promo_point.id +
                            "' min='1' value='" +
                        ...
                        let td_menu_price = $("<td align='right'>" +
                            "<span id='price_show_" + data.promo_point.menu.id + "_" + data.promo_point
                            .id + "'>-</span>" +
                            "<input type='hidden' id='order_item_price_" + data.promo_point.menu.id + "_" +
                            data.promo_point
                            .id +
                            "' name = 'order_item_promo[" +
                            data.promo_point.menu.id + "][price]'" +
                            "value = '' > " +
                            "</td>"
                        );

                        let point = parseInt(data.promo_point.point) * -1;
                        let td_menu_point = $("<td align='right'>" +
                            "<span id='point_show_" + data.promo_point.menu.id + "_" + data.promo_point
                            .id + "'>" +
                            currencyFormat(point) +
                            "</span>" +
                            "<input type='hidden' id='order_item_point_" + data.promo_point.menu.id + "_" +
                            data.promo_point
                            .id +
                            "' name = 'order_item_promo[" +
                        ...
                        let td_menu_action = $(
                            ...
                            "<input type='hidden' class='form-control' name='order_promo_item_check[]' value='" +
                            data.promo_point.menu.id + "_" + data.promo_point.id + "'>" +
                            "</td>"
                        );
                        ...
                        let last_qty = $("#order_item_qty_" + data.promo_point.menu.id + "_" + data.promo_point
                            .id).val();
                        let total_qty = parseInt(last_qty) + parseInt(data.promo_point.qty);
                        let real_qty = total_qty / parseInt(data.promo_point.qty)
                        let total_point = (parseFloat(data.promo_point.point) * -1) * real_qty;

                        $('#point_show_' + data.promo_point.menu.id + "_" + data.promo_point
                            .id).html(currencyFormat(total_point));
                        $('#order_item_qty_' + data.promo_point.menu.id + "_" + data.promo_point
                            .id).val(total_qty);
                        $('#order_item_point_' + data.promo_point.menu.id + "_" + data.promo_point
                            .id).val(total_point);
                    }
                    accumulationItem();
                }).fail(function(xhr, status, error) {
    ...
    function accumulationItem() {
        ...
        if (type == 0) {
            ...
            $("input[name='order_promo_item_check[]']")
                .map(function() {

                    total_point += parseFloat($('#order_item_point_' + $(this).val())
                        .val());
                    minus_point += parseFloat($('#order_item_point_' + $(this).val())
                        .val());
                });

            let customer_point = parseFloat($('#customer_point').val());
            let last_point = customer_point + minus_point;
